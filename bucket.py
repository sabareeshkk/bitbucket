...
class Bitbucket():
    ...
    def get_commits(self, username, repository, page=1):
        ...
        print query_params
        query_url = 'https://bitbucket.org/api/2.0/repositories/%s/%s/commits?%s' % (username, repository, query_params)
        print query_url
        result = self._get_result(query_url)
        return result

    def get_comments(self, username, repository, revision):
        query_url = 'https://bitbucket.org/api/2.0/repositories/%s/%s/commit/%s/comments' % (username, repository, revision)
        result = self._get_result(query_url)
        return result
